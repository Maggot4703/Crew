...
try:
    ...
    if VENV_PATH.exists():
        ...
        subprocess.run(
            [str(pip_exe), "install", "requests", "beautifulsoup4", "html2text"],
            check=False,
        )
        try:
            import html2text
...
class DocumentationFetcher:
    """
    Fetches and organizes documentation for a list of subjects from online sources and local files.

    Features:
    - Fetches theory, usage, and examples for each subject.
    - Supports multiple output formats and resume functionality.
    - Integrates with Crew Manager GUI and CLI.
    - Handles web scraping dependencies and user agent rotation.
    """

    def __init__(self, base_dir=DEFAULT_BASE_DIR, use_web=True, delay=3):
        """
        Initialize the DocumentationFetcher.

        Args:
            base_dir (str or Path): Base directory for output files.
            use_web (bool): Whether to fetch documentation from the web.
            delay (int): Delay (in seconds) between web requests.
        """
    ...
    def create_content(self, subject, level, ctype):
        ...
        if ctype == "theory":
            ...
            content += (
                f"Generated stub content for {subject} {level} usage instructions.\n\n"
            )
            content += "## Further Reading\n"
            for u in urls:
                content += f"- {u}\n"
        ...
        for subj in subjects:
            ...
            for lvl in LEVELS:
                ...
                for ct in CONTENT_TYPES:
                    ...
                    index_links.append(
                        f'<a href="{lvl}/{ct}.txt">{lvl.capitalize()} {ct.capitalize()}</a>'
                    )
            # write HTML index with hyperlinks
            html = "<html><body>\n" + "<br/>\n".join(index_links) + "\n</body></html>"
            (subj_dir / "index.html").write_text(html)
        # Record progress (append unique)
        self.progress["completed"] = list(
            dict.fromkeys(self.progress["completed"] + [subj])
        )
        self._save_progress()
