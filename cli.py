...
def create_cli_parser():
    ...
    parser_grid_image.add_argument("image_path")
    parser_grid_image.add_argument("output_path")
    parser_grid_image.add_argument("--grid-width", type=int, default=42)
    parser_grid_image.add_argument("--grid-height", type=int, default=32)
    parser_grid_image.add_argument("--grid-color", type=str, default="lightgrey")
    parser_grid_image.add_argument("--labels", action="store_true")
    parser_grid_image.add_argument(
        "--output-format", choices=["png", "jpg", "jpeg", "webp"]
    )
    parser_grid_image.add_argument("--quality", type=int, default=95)
...
def handle_grid_image(args, logger):
    logger.info("Running grid-image with path: %s", args.image_path)
    ...
    if image is None:
        print("[ERROR] Failed to overlay grid.", file=sys.stderr)
        return 1
