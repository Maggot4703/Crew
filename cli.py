...
def create_cli_parser() -> argparse.ArgumentParser:
    ...
    deepseek_code = subparsers.add_parser(
        "deepseek-code",
        help="Send a prompt to DeepSeek Code server and print the response.",
    )
    deepseek_code.add_argument(
        "prompt", help="Prompt or code to send to DeepSeek Code server."
    )
...
def run_cli(args: argparse.Namespace) -> int:
    ...
    if args.command == "grid-image":
        ...
        if image is None:
            print(
                f"[ERROR] Could not process image: {args.image_path}. Please check the file path and format.",
                file=sys.stderr,
            )
            return 1
        ...
        try:
            ...
            print(
                f"[ERROR] Failed to save output image: {args.output_path}. {e}",
                file=sys.stderr,
            )
            return 1
        return 0
    ...
    if args.command == "grid-folder":
        ...
        if not saved:
            print(
                "[ERROR] No images were processed. Please check the input directory and file formats.",
                file=sys.stderr,
            )
            return 1
