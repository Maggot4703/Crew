...
from ctypes import (CFUNCTYPE, POINTER, Structure, c_char, c_char_p, c_double,
                    c_int, c_long, c_longlong, c_ubyte, c_uint, c_void_p, cast,
                    string_at)

from ijson import backends, common
...
def _get_callback_data(yajl_version):
    return [
        ...
        (
            "integer",
            "number",
            C_LONG if yajl_version == 1 else C_LONGLONG,
            lambda v: int(v),
        ),
        ...
        (
            ...
            "number",
            C_STR,
            lambda v, l: common.integer_or_decimal(string_at(v, l).decode("utf-8")),
        ),
...
def make_callbaks(send, use_float, yajl_version):
    ...
    return (
        Callbacks(*[_make_callback(send, use_float, *data) for data in callback_data]),
        _all_cfunct_types,
    )
