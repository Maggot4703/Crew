...
def big_longstr_object(n):
    str_template = (
        b"value that is very long and should cause a bit less of JSON parsing"
    )
    return (
        b"{"
        + b",\n".join([b'"key_%d": "%s"' % (i, str_template) for i in range(n)])
        + b"}"
    )
...
def object_with_10_keys(n):
    ...
    return b"[" + b",\n".join(template for _ in range(n)) + b"]"
...
def run_benchmarks(args, benchmark_func=None, fname=None):
    ...
    for backend_name, backend in args.backends.items():
            ...
            def run(reader):
                try:
                    loop.run_until_complete(
                        _run_async(method, reader, *method_args, **method_kwargs)
                    )
                finally:
                    loop.close()
        ...
        print(
            "%.3f, %s, %s, %s, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f" % results
        )
...
def main():
    ...
    parser.add_argument(
        ...
        "--size",
        type=int,
        help="Size of JSON content; actual size in bytes might differ, defaults to %d"
        % DEFAULT_N,
        default=DEFAULT_N,
    )
    parser.add_argument(
        ...
        "--iterations",
        type=int,
        help="How many times each method should be tested, defaults to %d"
        % DEFAULT_ITERATIONS,
    ...
    parser.add_argument(
        "-b",
        "--benchmarks",
        type=parse_benchmarks,
        help="Comma-separated list of benchmarks to include, defaults to %s"
        % ALL_BENCHMARKS,
    ...
    parser.add_argument(
        ...
        "--backends",
        type=parse_backends,
        help="Comma-separated list of backends to include, defaults to %s"
        % ALL_BACKENDS,
    ...
    parser.add_argument(
        "-m",
        "--method",
        choices=["basic_parse", "parse", "kvitems", "items"],
        help="The method to benchmark",
        default="basic_parse",
    )
    parser.add_argument(
        "-c",
        ...
        default=False,
        dest="run_coro",
        help="Benchmark coroutine methods",
    )
    parser.add_argument(
        "-a",
        "--async",
        ...
        default=False,
        dest="run_async",
        help="Benchmark asyncio-enabled methods",
    )
    parser.add_argument(
        "-p", "--prefix", help="Prefix (used with -M items|kvitems)", default=""
    )
    ...
    print(
        "#mbytes, method, test_case, backend, time_min, time_avg, time_median, time_max, mb_per_sec_min, mb_per_sec_avg, mb_per_sec_median, mb_per_sec_max"
    )
    if args.inputs:
