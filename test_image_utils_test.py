...
def test_overlay_grid(tmp_path):
    ...
    img.save(img_path)
    result = image_utils.overlay_grid(
        str(img_path), grid_color="green", grid_size=(10, 10), show_labels=True
    )
    assert result is not None
    assert isinstance(result, Image.Image)
...
def test_process_images(tmp_path):
    ...
    img.save(img_path)
    results = image_utils.process_images(
        str(input_dir),
        str(output_dir),
        grid_size=(5, 5),
        grid_color="red",
...
def test_crop_from_annotations(tmp_path):
    ...
    output_dir.mkdir()
    results = image_utils.crop_from_annotations(
